    ...
    <div class="col-sm-8">
        <select name="lesson_id" class="form-control placeholder-single" id="lesson_id"
            ...
            <option value="<?php echo html_escape($single->lesson_id); ?>"
                <?php echo (($get_assigneditdata->lesson_id == $single->lesson_id) ? 'selected' : ''); ?>>
    ...
    <div class="col-sm-8">
        <select name="section_id" class="form-control placeholder-single" id="section_id"
            ...
            <option value="<?php echo html_escape($single->section_id); ?>"
                <?php echo (($get_assigneditdata->section_id == $single->section_id) ? 'selected' : ''); ?>>
    ...
    <div class="col-sm-8">
        <select name="exam_id" class="form-control placeholder-single" id="exam_id"
            ...
            <option value="<?php echo $exam->exam_id; ?>"
                <?php echo (($get_assigneditdata->exam_id == $exam->exam_id) ? 'selected' : ''); ?>>
                <?php echo $exam->name; ?>
            </option>
            <?php } ?>
        </select>
...
<div class="form-group row ">
    <div class="offset-3 col-md-2">
        <button type="button" class="btn btn-success w-md m-b-5"
            onclick="assignExam('<?php echo html_escape($course_id); ?>','<?php echo html_escape($get_assigneditdata->id); ?>')"><?php echo display('update'); ?></button>
    </div>
</div>
<?php echo form_close(); ?>