...
class Module extends MX_Controller {
    ...
    public function module_list() {
        $data['title'] = display('module_list');
        $modulesnames = $this->module_model->get_installed_module_names();
        $json_module = $this->module_model->search_available_modules();
//        dd($modulesnames);
        $modules = [];
        if (!empty($json_module)) {
            $data['modules'] = json_decode($json_module);
        }
        $data['installed'] = $modulesnames;
    ...
    public function purchase_addons() {
        $data['title'] = display('purchase_verification');
        $data['module'] = "dashboard";
        $data['page'] = "module/purchase_addons";
        echo Modules::run('template/layout', $data);
    }

    // Unzip Files
    public function unzip_files($zip_file) {
        $file = 'file.zip';
    ...
    private function create_module_json() {
        ...
        if (isset($HmvcMenu) && $HmvcMenu != null && sizeof($HmvcMenu) > 0) {
            ...
            $cache_file = './assets/json/addon.json';
            $itemlist = json_encode($jsonvalue);
            file_put_contents($cache_file, $itemlist);
        }
    }

//    ============== its for verify_purchase_request =============
    public function verify_purchase_request() {
        ...
        if (!empty($purchase_key) && !empty($itemtype)) {
            ...
            $result = $this->module_model->purchase_new_module($getdata);
//            dd($result);
...
//                            'image' => 'application/modules/' . $result->identity . '/assets/images/thumbnail.jpg',
//                            'directory' => $result->identity,
//                            'purchase_key' => $purchase_key,
//                            'status' => 1,
//                        );
////                        dd($m_p_data);
//                        $this->db->insert('module', $m_p_data);
                        // Module Json File
    ...
    public function upload() {
        ...
        redirect("dashboard/module/module_list");
    }
                ...
                if (($HmvcConfig[$directory]["_database"] === true) && !empty($HmvcConfig[$directory]["_tables"]) && is_array($HmvcConfig[$directory]["_tables"]) && sizeof($HmvcConfig[$directory]["_tables"]) > 0) {
                    ...
                    if (strlen($error) > 0) {
                        ...
                        if ($this->module_model->create($moduleData)) {
                            // add a install flag
                            @file_put_contents(
                                            APPPATH . 'modules/' . $directory . '/assets/data/env', date('d-m-Y')
                            );
