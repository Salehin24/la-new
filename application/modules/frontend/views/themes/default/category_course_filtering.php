        ...
        <div class="course-card course-card-shadow rounded-6 bg-white position-relative overflow-hidden">
          <div class="position-relative overflow-hidden bg-prussian-blue">
            ...
            <div class="align-items-center top-0 d-flex fs-12 position-absolute px-3 py-2 start-0 text-white w-100 z-index-2">
                ...
                if(!empty($category_type)){
                    if($category_type==1){
                        ...
                        if($category_course->tagstatus==4){
                            ...
                            echo "New";
                        }elseif($category_course->tagstatus==2){
                            echo "Best Seller";
                        }elseif($category_course->tagstatus==1){
                            echo "Recomended";
                        }else{          
                        } 
                    }
                ...
                // if($category_course->is_new==1){ echo "New";}
                ?></span>
                ...
                <?php
                    ...
                    if (!$coursesaved_checked){
                        if ($user_type == 4) {?>
                        <img onclick="get_coursesaveloop(1, '<?php echo $category_course->course_id; ?>')" src="<?php echo base_url('application/modules/frontend/views/themes/' . html_escape(get_activethemes()->name) . '/assets/img/bookmark-1.png'); ?>" class="img-fluid ms-auto " alt="" style="cursor: pointer;">
                    <?php } else {
                        if($user_type != 5){
                        ?>
...
<div class="text-center mt-5">
    <div id="load<?php echo  $category_course->id ;?>">
        <button type="button" class="btn btn-lg btn-dark-cerulean load"  onClick="category_course_filtering_loadmore('<?php echo $category_course->id ;?>')"  id="<?php echo  $category_course->id ;?>">
            Browse more Courses
...
<script>
  "use strict";
    ...
    //         cache: false,
    //         data: {lastid: ids, category_id: category_id, enterprise_shortname: enterprise_shortname, csrf_test_name: CSRF_TOKEN},
    ...
    //     });
    // });
    // $("#course_subscription").hide();
    // $('#course_subscription').removeClass('d-flex');
    // $( ".hideClass" ).each(function( index ) {
    //   var p_course_id=$("#p_course_ids_"+index).val(); 
	// //   alert(p_course_id);
