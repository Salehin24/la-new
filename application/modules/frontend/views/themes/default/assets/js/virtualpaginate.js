...
virtualpaginate.prototype.navigate = function (keyword) {
    ...
    for (var p = 0; p < this.paginatediv.length; p++) { //loop through all pagination DIVs
        if (this.flatviewpresent)
            this.flatviewlinks[p][prevlinkindex].className = "page-numbers" //"Unhighlight" previous page (before this.currentpage increments)
        if (this.selectmenupresent)
            this.selectmenu[p].selectedIndex = this.currentpage
        if (this.flatviewpresent)
            this.flatviewlinks[p][this.currentpage].className = "page-numbers current" //"Highlight" current page
    }
    if (!this.wraparound) {
...
virtualpaginate.prototype.paginate_output_flatview = function (flatviewcontainer, divpos, anchortext) {
    var flatviewhtml = "";
    var myclass = "page-numbers";
    var anchortext = anchortext || new Array()
    for (var i = 0; i < this.pagecount; i++) {
        if (typeof anchortext[i] != "undefined") //if custom anchor text for this link exists
            flatviewhtml += '<a href="#flatview" rel="' + i + '" class="' + myclass + '">' + anchortext[i] + '</a> ' //build pagination link using custom anchor text
        else
            flatviewhtml += '<a href="#flatview" rel="' + i + '" class="' + myclass + '">' + (i + 1) + '</a> ' //build  pagination link using auto incremented sequential number instead
    }
...
virtualpaginate.prototype.paginate_build_flatview = function (flatviewcontainer, divpos, anchortext) {
    ...
    this.flatviewlinks[divpos][this.currentpage].className = "page-numbers current" //"Highlight" current flatview link
